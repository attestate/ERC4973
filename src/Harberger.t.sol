...
contract HarbergerTest is DSTest {
  PropertyMock p;
  ContinuousAuctioneer ca;

  function setUp() public {
    p = new PropertyMock();

    ca = new ContinuousAuctioneer();
  }
  ...
  function testBidOnAnOffer() public {
    ...
    uint256 tokenId = 0;
    uint256 offerId0 = ca.create{value: startPrice}(
      taxRate,
      token,
      tokenId
    );
    ...
    ca.bid{value: 1.1 ether}(offerId0);
    ...
    assertEq(address(ca).balance, 1.1 ether);
  }
  ...
  function testFailBidWithFalsePrice() public {
    ...
    ca.bid{value: 0.1 ether}(offerId0);
  }
  ...
  function testCreateOffer() public {
    ...
    assertEq(offerId0, 0);
    uint256 offerId1 = ca.create{value: startPrice}(
      taxRate,
      token,
      tokenId
    );
