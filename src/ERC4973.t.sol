...
contract AccountAbstraction is ERC1271Mock {
    ...
    function give(address collection, address to, string calldata uri, bytes calldata signature)
        external
        virtual
        returns (uint256)
    {
    ...
    function take(address collection, address from, string calldata uri, bytes calldata signature)
        external
        virtual
        returns (uint256)
    {
...
contract AccountBoundToken is ERC4973 {
    ...
    function getHash(address from, address to, string calldata tokenURI) public view returns (bytes32) {
...
contract ERC4973Test is Test {
    ...
    function testGiveWithRejectingERC1271Contract() public {
        ...
        abt.give(to, tokenURI, signature);
    }
    ...
    function testTakeWithRejectingERC1271Contract() public {
        ...
        vm.expectRevert(bytes("_safeCheckAgreement: invalid signature"));
        abt.take(from, tokenURI, signature);
    }
    ...
    function testGiveWithApprovingERC1271Contract() public {
        ...
        uint256 tokenId = abt.give(to, tokenURI, signature);
        assertEq(abt.balanceOf(to), 1);
        assertEq(abt.tokenURI(tokenId), tokenURI);
        assertEq(abt.ownerOf(tokenId), to);
    }
    ...
    function testTakeWithApprovingERC1271Contract() public {
        ...
        uint256 tokenId = abt.take(from, tokenURI, signature);
        assertEq(abt.balanceOf(to), 1);
        assertEq(abt.tokenURI(tokenId), tokenURI);
        assertEq(abt.ownerOf(tokenId), to);
    }
    ...
    function testTakeWithDifferentTokenURI() public {
        ...
        vm.expectRevert(bytes("_safeCheckAgreement: invalid signature"));
        uint256 tokenId = abt.take(passiveAddress, tokenURI, signature);
        assertEq(0, tokenId);
    }
    ...
    function testGiveWithUnauthorizedSender() public {
        ...
        vm.expectRevert(bytes("_safeCheckAgreement: invalid signature"));
        uint256 tokenId = abt.give(unauthorizedTo, tokenURI, signature);
        assertEq(0, tokenId);
    }
    ...
    function testTakeWithUnauthorizedSender() public {
        ...
        vm.expectRevert(bytes("_safeCheckAgreement: invalid signature"));
        uint256 tokenId = abt.take(unauthorizedFrom, tokenURI, signature);
        assertEq(0, tokenId);
    }
    ...
    function testGiveEOA() public {
        ...
        bytes memory expected =
            hex"238e1616c507f9779469b0276eef73a3a438b65706ca18c6ab38062c588674f9719c9f5412b0379e7918f19da1de71b9370ed9917fadcb6690e71f5a1de24816";
    ...
    function testGiveAndUnequipAndRegive() public {
        ...
        assertEq(abt.balanceOf(to), 0);
        uint256 tokenId2 = abt.give(to, tokenURI, signature);
        assertEq(abt.balanceOf(to), 1);
    ...
    function testTakeAndUnequipAndRetake() public {
        ...
        abt.unequip(tokenId);
        assertEq(abt.balanceOf(to), 0);
        uint256 tokenId2 = abt.take(passiveAddress, tokenURI, signature);
        assertEq(abt.balanceOf(to), 1);
        assertEq(abt.tokenURI(tokenId2), tokenURI);
        assertEq(abt.ownerOf(tokenId2), to);
    }
    ...
    function testTakeWithAlreadyUsedVoucher() public {
        ...
        abt.take(passiveAddress, tokenURI, signature);
    ...
    function testPreventGivingToSelf() public {
        ...
        aa.give(address(abt), to, tokenURI, signature);
    }
    ...
    function testPreventTakingToSelf() public {
        ...
        aa.take(address(abt), from, tokenURI, signature);
    }
}
